...
def main(
    ...
    save_model_at = "models/%s/%s" % (expe_name, saveadr)
    model_save_adr = "%s/checkpoint.pth.tar" % save_model_at
    ...
    if arch == "iresnetauto":
        model = IResNetAuto
    if arch == "resnetmemory101":
        model = ResNetMemory101
    if arch == "resnetmemorycustom":
        model = ResNetMemoryCustom
    if not torch.cuda.is_available():
        print("using CPU, this will be slow")
    if arch == "iresnetauto":
        model = model(n_layers=n_layers)
    elif arch == "resnetmemory101":
        model = model(zero_init_residual=False, use_backprop=use_backprop)
        print("zero_init_residual")
    elif arch == "resnetmemorycustom":
        model(
            zero_init_residual=True,
            layers=[3, 8, n_layers, 3],
